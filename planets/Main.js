...
const github = 'https://github.com/gnpaone';
    ...
    <div className='relative z-20'>
          <div className='fixed w-full bg-white z-20 items-center justify-center hidden lg:flex'>
            {planets.map((planet, index) => 
                <button key={index} onClick={() => navigateToPlanet(planet.name)} id={planet.name} className={`${planet.name == currentPlanet ? 'text-black': 'text-gray-500'} navbtns mx-2 px-2 py-2 hover:shadow hover:bg-gray-200`}>{planet.name}</button>
        ...
        <div className='fixed flex items-center justify-center h-[7rem] z-20 bottom-0 w-full lg:hidden'>
          <div className='bg-white rounded-full w-[50%] lg:w-[30%] mx-auto flex items-center justify-center'>
               <button id={backValue} className='navbtns relative w-6 h-6 rounded-full ' onClick={() => navigateToPlanet(backValue)}><Image id={backValue} layout='fill' src={'/back.svg'} /></button> 
               <span className='mx-4 text-3xl tracking-wide shadow-lg'>{currentPlanet}</span> 
               <button id={nextValue} className='navbtns relative w-6 h-6 rounded-full' onClick={() => navigateToPlanet(nextValue)}><Image id={nextValue} layout='fill' src={'/next.svg'} /></button> 
          </div>
        </div>
